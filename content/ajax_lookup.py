...
from content.context_processors import context_championship_id
from content.models import Championship
...
class PlaceLookup(object):
    ...
    def get_query(self, q, request):
        ...
        # If possible, also filter by the city of the championship.
        context = context_championship_id(request)
        championship_id = context['context_championship_id']
        if championship_id is not None:
            championship = Championship.objects.get(pk=championship_id)
            queryset = queryset.filter(city=championship.city_id)
        return queryset
