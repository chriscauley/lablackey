...
from lablackey.content.models import ItemList, Copy, DesignImage
...
@register.simple_tag
def print_list(context,args=None):
    values = load_defaults({'columns':1, 'template':"itemlist.html"},args)
    l, new = ItemList.objects.get_or_create(context=context)
    if new:
        l.name = context.replace("-"," ").title()
        l.save()
    values['itemlist'] = l
    return render_to_string(values['template'],values)
