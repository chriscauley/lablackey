...
@register.filter
def truncateString(string, length, ellipsis='&hellip;'):
    """Truncate HTML string, preserving tag structure and entities."""
    if string:
        length = int(length)
        output_length = 0
        i = 0
        ...
        while output_length < length and i < len(string):
            ...
            if c == '<':
                ...
                if i in pending_close_tags:
                    # just pop and skip if it's closing tag we already
                    # knew about.
                    ...
                    if match:
                        ...
                        # save the end tag for possible later use
                        # if there is one
                        match = re.search(
                            r'(</' + tag + '[^>]*>)',
                            string[i:],
                            re.IGNORECASE)
                        if match:
                            pending_close_tags[i + match.start()] = (
                                match.groups()[0])
                    else:
                ...
                # this is either a weird character or just '&',
                # both count as 1
                output_length += 1
            else:
